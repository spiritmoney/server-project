...
contract Swap is OwnerPausable, ReentrancyGuard {
    ...
    constructor(
        ...
        for (uint8 i = 0; i < _pooledTokens.length; i++) {
            ...
            precisionMultipliers[i] =
                10 ** uint256(36).sub(uint256(_decimals[i]));
    ...
    function initialize(
        ...
        require(
            _assetTypes.length == swapStorage.pooledTokens.length &&
                _assetTypes.length == _oracles.length,
            "length mismatch"
        );
        ...
        uint256[2][] memory _lastPrices = new uint256[2][](
        ...
        for (uint8 i = 0; i < _pooledTokens.length; i++) {
            ...
            if (_assetTypes[i] == 3) {
                ...
                address underlyingAsset = IERC4626(address(_pooledTokens[i]))
                    .asset();
                _scaleFactor[i] =
                    10 **
    ...
    function lpTokenAddress() external view returns (address _LPToken) {
